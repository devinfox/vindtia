...
import { FASHION_KNOWLEDGE } from "./fashion-knowledge";
...
// Build subculture reference for the AI to use
...
/**
 * Generates an AI description for a product and stores it in the database.
 * This creates a CLASSIFICATION-FOCUSED description for AI stylist matching,
 * not marketing copy. It references fashion subcultures, eras, and aesthetics.
 */
export async function generateProductAIDescription(
  productId: string,
  ...
  if (!imageUrls || imageUrls.length === 0) {
    console.log("No images provided for AI description generation");
    return;
  }
...
Analyze these vintage fashion images and create a CLASSIFICATION DESCRIPTION for our AI stylist. Identify subcultures, eras, aesthetics, and all relevant matching keywords. Be thorough with classification tags.`;
    ...
    if (!aiDescription) {
      console.error("Failed to generate AI description - no content returned");
      return;
    }
    ...
    // Update the product in database using admin client to bypass RLS
    ...
    if (error) {
      console.error("Failed to save AI description to database:", error);
      return;
    }

    console.log(`AI classification description generated successfully for product ${productId}`);
  } catch (error) {
